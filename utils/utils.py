...
def save_model(model, path, epoch, mode, optimizer=None):
    ...
    Path(path).mkdir(parents=True, exist_ok=True)  # create a dictionary
    # ipdb.set_trace()
    ...
    if mode == "checkpoint":
        ...
        torch.save(
            model.state_dict(), str(path + f"best_{mode}_epoch{epoch}_{localtime}.pth")
        )
...
def train_val_test(
    ...
    for i, (image, labels, name) in enumerate(tbar):
        ...
        image_patches = image.unfold(2, crop_size, crop_size).unfold(
        ...
        image = (
            image_patches.permute(0, 2, 3, 1, 4, 5)
            .reshape(-1, C, crop_size, crop_size)
            .contiguous()
        )

        # labels的形状为B,H,W, unfold之后形状为[B, ratio, ratio, H/ratio, W/ratio]

        labels_patches = labels.unfold(1, crop_size, crop_size).unfold(
            2, crop_size, crop_size
        )
        ...
        batch_metrics = metric_collection.forward(
            preds.float(), labels.int().unsqueeze(1)
        )  # compute metric
    ...
    print(f"{mode} f1score: {epoch_metrics['f1score']} iou: {epoch_metrics['iou']}")
    print(f"{mode} epoch loss: {epoch_loss}")

    for k in epoch_metrics.keys():
        log_wandb.log(
            {f"epoch_{mode}_{str(k)}": epoch_metrics[k], "epoch": epoch}
        )  # log epoch metric
    metric_collection.reset()
    log_wandb.log({f"epoch_{mode}_loss": epoch_loss, "epoch": epoch})  # log epoch loss
